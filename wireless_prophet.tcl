...
Agent/MessagePassing/Prophet instproc meetNodeProb {nodea nodeb } {
	...
	set delivery_pred($nodea,$nodeb) [format "%.4f" [expr ($delivery_pred($nodea,$nodeb) \
	+((1 - $delivery_pred($nodea,$nodeb) )) * $Pinit)]]
	set current_time [format "%.2f" [expr {double(round(100*[$ns now]))/100}]]
	set delivery_pred($nodea,$nodeb,"time") $current_time	
	
  	writeFT "\n$delivery_pred($nodea,$nodeb,"time") -meetNodeProb- => prob($nodea,$nodeb): \
...
Agent/MessagePassing/Prophet instproc agingNodeProb {nodea nodeb } {
	...
	set delivery_pred($nodea,$nodeb,"time") $current_time	
 	...
 	writeFT "\n$delivery_pred($nodea,$nodeb,"time") -agingNodeProb- => prob($nodea,$nodeb): \
 	$delivery_pred($nodea,$nodeb)"
...
Agent/MessagePassing/Prophet instproc transitiveNodeProb {nodea nodeb nodec} {
	...
	set current_time [format "%.2f" [expr {double(round(100*[$ns now]))/100}]]
	set delivery_pred($nodea,$nodec) [format "%.4f" [expr ($delivery_pred($nodea,$nodec) \
	 + (1 - $delivery_pred($nodea,$nodec))* $delivery_pred($nodea,$nodeb) \
	 * $delivery_pred($nodeb,$nodec) * $Beta )]]
	set delivery_pred($nodea,$nodec,"time") $current_time	

	writeFT "\n$delivery_pred($nodea,$nodec,"time") -transitiveNodeProb- => prob($nodea,$nodec):\
...
Agent/MessagePassing/Prophet instproc recv {source sport size data} {
	...
	if {$a($packet_receiver,'type') == 0} { 
		...
		#normal node
		writeFT "\n$current_time -recvPacket- =>pkt_id:$packet_id (NormalNode) \
		pkt_source:$source pkt_receiver:$packet_receiver"
		...
		if { [lsearch $friends($packet_receiver) $source] == -1 } { 
			...
			writeFT "\n$current_time -addNewFriend- => node:$packet_receiver \
			friends:$friends($packet_receiver)"
		}
		...
		for {set x 0} {$x < $num_nodes} {incr x} {
			if { ([lsearch $friends($packet_receiver) $x] == -1) && ( $x != $source) && ( $x != $packet_receiver ) 
		...
		writeFT "\n$current_time -recvPacket- => pkt_id:$packet_id (SpyNode) \
		pkt_source:$source pkt_receiver:$packet_receiver"
